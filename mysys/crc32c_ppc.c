...
#ifdef HAVE_POWER8
unsigned int __crc32c_vpmsum(unsigned int crc, unsigned char const *p,
                            unsigned long len);

static uint32_t crc32c_vpmsum(uint32_t crc, unsigned char const *data,
                             unsigned len) {
...
uint32_t crc32c_ppc(uint32_t crc, unsigned char const *data, unsigned len) {
  ...
  if (!data) {
    ...
    bzero(buf2, len);
    crc = crc32c_vpmsum(crc, buf2, len);
    free(buf2);
  } else {
    crc = crc32c_vpmsum(crc, data, (unsigned long)len);
  }
  return crc;
}
